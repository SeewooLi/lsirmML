...
#
#       L1L2 <- L1L2_lsirm_cpp0(E$e.response[i,,], item, E$grid)
#       # IM[index,index] <- L1L2$IM
#       if(model== 1){
#         diff <- as.vector(
#           solve(L1L2$IM[-1,-1] + diag(1/(sds^2))) %*%
#             (L1L2$gradient[-1] - (c(0,estimated_item[i,-(1:2)]-0)/c(1,sds[-1]^2)))
#         )
#         estimated_item[i,-1] <- estimated_item[i,-1] + diff * contrast_m[i,-1] / 2
#       } else if(model == 2){
#         diff <- as.vector(
#           solve(L1L2$IM + diag(c(0, 1/(sds^2)))) %*%
#             (L1L2$gradient - (c(0,0,estimated_item[i,-(1:2)]-0)/c(1,1,sds[-1]^2)))
#         )
...
#     iter <- iter + 1
#
#     E <- Estep_cpp0(data, initial_item, grid, prior)
